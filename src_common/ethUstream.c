...
static void processNetworkID(txContext_t *context) {
    if (context->currentFieldIsList) {
        PRINTF("Invalid type for RLP_DATA\n");
        THROW(EXCEPTION);
    }
...
static void processFromShardID(txContext_t *context) {
    if (context->currentFieldIsList) {
        PRINTF("Invalid type for RLP_DATA\n");
        THROW(EXCEPTION);
    }
...
static void processToShardID(txContext_t *context) {
    if (context->currentFieldIsList) {
        PRINTF("Invalid type for RLP_DATA\n");
        THROW(EXCEPTION);
    }
...
static void processGasTokenID(txContext_t *context) {
    if (context->currentFieldIsList) {
        PRINTF("Invalid type for RLP_DATA\n");
        THROW(EXCEPTION);
    }
...
static void processTransferTokenID(txContext_t *context) {
    if (context->currentFieldIsList) {
        PRINTF("Invalid type for RLP_DATA\n");
        THROW(EXCEPTION);
    }
...
static parserStatus_e processTxInternal(txContext_t *context) {
    for (;;) {
        ...
        if (customStatus == CUSTOM_NOT_HANDLED) {
            ...
            //     processV(context);
            //     break;
            case TX_RLP_NETWORKID:
                processNetworkID(context);
                break;
            case TX_RLP_FROMSHARDID:
                processFromShardID(context);
                break;
            case TX_RLP_TOSHARDID:
                processToShardID(context);
                break;
            case TX_RLP_GASTOKENID:
                processGasTokenID(context);
                break;
            case TX_RLP_TRANSFERTOKENID:
                processTransferTokenID(context);
                break;
            default:
